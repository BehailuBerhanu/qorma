...
function Sidebar({ active, setActive, userName, userEmail, onSignOut }: { active: string; setActive: (label: string) => void; userName: string; userEmail: string; onSignOut: () => void }) {
  return (
    <aside className="hidden w-[248px] shrink-0 flex-col bg-[#03251d] px-4 py-6 text-white lg:flex">
      ...
      <div className="mt-8 flex items-center gap-3 px-2"><div className="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-700 text-lg">{userName.slice(0, 2).toUpperCase()}</div><div className="min-w-0"><div className="text-sm font-medium">Hi, {userName}</div><div className="truncate text-xs text-emerald-100/70">{userEmail}</div></div><button onClick={onSignOut} className="ml-auto text-xs text-emerald-100/70 hover:text-white">Sign out</button></div>
    </aside>
  )
}
...
export default function QormaDashboard({ user }: { user: { name: string; email: string } }) {
  const router = useRouter()
  const [active, setActive] = useState('Dashboard')
  const handleSignOut = async () => { await signOut(); router.push('/sign-in'); router.refresh() }
