  ...
  return (
    <main className="landing-page">
      ...
      <section className="hero landing-container">
        <div className="hero-copy"><span className="kicker"><span className="et-flag" role="img" aria-label="Ethiopian flag"><span className="et-flag-stripe et-green"/><span className="et-flag-stripe et-yellow"/><span className="et-flag-stripe et-red"/><span className="et-flag-circle"/></span> Built for Ethiopian students</span><h1>Master every <span>question.</span></h1><p>Practice past exams, understand every mistake with AI explanations, and build the confidence to ace your next entrance exam.</p><div className="hero-buttons"><Link href="/sign-up" className="primary-button">Start practicing free <ArrowRight size={17} /></Link><a href="#features" className="secondary-button">Explore past exams</a></div><div className="proof"><div className="avatars"><i>AB</i><i>MK</i><i>HT</i><i>DN</i><i>+</i></div><div><strong>10,000+ students</strong><span>are already improving</span></div></div></div>
      ...
      <div className="benefit-marquee" aria-label="Qorma benefits"><div className="marquee-track">{['Past exam library','AI explanations','Smart practice','Progress tracking','Offline learning','Performance analytics','Past exam library','AI explanations','Smart practice','Progress tracking','Offline learning','Performance analytics'].map((item, index) => <span key={`${item}-${index}`}><b>✓</b>{item}</span>)}</div></div>
